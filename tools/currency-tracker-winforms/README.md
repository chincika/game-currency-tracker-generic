...
旧版固定账号 JSON 可以直接加载或通过 `导入备份` 导入，程序会自动转换为新版 `schemaVersion: 2` 数据。

主界面的手动更新区、当前分组合计区、历史查询区可以通过分隔条拖拽调整大小，调整结果会保存到设置文件。

历史查询中提供 `周收益图` 标签页，会按每周周六作为结算点绘制收益折线图，并可点击 `打开大图` 弹出独立窗口查看。
